...
$(document).ready(function(){
	...
	$('.rep_n').click(function(){
		...
		location.href="${pageContext.request.contextPath }/notice/detail?num="+${n.num};
	});
});
		...
		$.ajax({
			...
			success:function(data){
				...
				for(i=0;i<arr.length;i++){
					...
					str +="<td style='width:5%'><input type='button' class='rep_n' rnum='${nr.num }' value='x'></td>"
					str +="</tr></table>";				
				}
...
<tr><th>작성자</th><td><input type="text" name="writer" value="${n.writer }" style='width:100%' readonly></td></tr>
<c:choose>
<c:when test="${sessionScope.id eq n.writer }">
<tr><th>공지명</th><td><input type="text" name="title" value="${n.title }" style='width:100%'></td></tr>
<tr><th>공지내용</th><td><textarea style="resize:none" name="content" id="summernote">${n.content }</textarea></td></tr>
</c:when>
<c:otherwise>
<tr><th>공지명</th><td><input type="text" name="title" value="${n.title }"  style='width:100%' readonly></td></tr>
<tr><th>공지내용</th><td><textarea style="resize:none" name="content" id="summernote" readonly>${n.content }</textarea></td></tr>
</c:otherwise>
</c:choose>
<tr>
<th></th>
...
<input type="submit" class="edit"num="${n.num }" value="수정" >
<input type="button" class="del" num="${n.num }" value="삭제" >
</c:if>
</td>
<tr>
<th>댓글작성</th>
...
<input type="text" id="content_${n.num }" style="width:94%">
<input type="button" class="rep" num="${n.num }" value="작성">
</td>
</tr>
<tr>
<th>댓글목록</th>
...
<tr>
	...
	<td style='width:15%'><input type="hidden" class="r_writer" value="${nr.writer }">${nr.writer }</td>
	<td style='width:80%'>${nr.content }</td>
	<c:if test="${nr.writer eq sessionScope.id}">
	<td style='width:5%'><input type="button" class="rep_n" rnum="${nr.num }" value="x"></td>
	</c:if>
</tr>
...
<c:if test="${count == 0 }"><tr><td>작성된 댓글이 없습니다</td></tr></c:if>
</table>
</td></tr>
</table>
</form>
